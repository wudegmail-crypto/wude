...
# 合同字段提取规则
# action: 直接取数 / 求和 / 多个日期取最早~最晚 / 多个日期取最晚
# source_field 以 __COMPOUND_ 开头时走复合计算
...
HARDCODED_RULES = {
    "重特大工程": [
        ...
        {"field": "数量",               "source_table": "次表1", "source_field": "采购数量",           "action": "求和",     "note": "合同下多条物资的采购数量汇总"},
        {"field": "单位",               "source_table": "次表1", "source_field": "计量单位",           "action": "直接取数", "note": "明细单位一致，取第一个"},
        ...
        {"field": "投标偏差（是/否）",     "source_table": "主表",  "source_field": "备注",               "action": "直接取数", "note": "从备注判断正偏差/负偏差/否"},
        {"field": "合同要求到货时间",      "source_table": "主表",  "source_field": "合同交货日期",       "action": "直接取数", "note": "直接取数"},
        {"field": "现场最终要求交货日期(系统)", "source_table": "次表1", "source_field": "合同最终交货日期", "action": "直接取数", "note": "单个日期直接取，多个相同取重复值，多个不同取最早~最晚"},
        ...
        {"field": "实际到货日期",         "source_table": "次表3", "source_field": "实际到货日期",     "action": "直接取数", "note": "单个日期直接取，多个相同取重复值，多个不同取最早~最晚"},
        {"field": "全部到货验收时间",       "source_table": "主表",  "source_field": "__COMPOUND_ACCEPTANCE__",    "action": "直接取数", "note": "到货比例100%后取最晚验收日期，否则填/"},
        ...
        {"field": "预付款支付时间",         "source_table": "次表2", "source_field": "__COMPOUND_PREPAY__",         "action": "直接取数", "note": "筛选付款类型=预付款，取实际付款日期"},
        {"field": "冻结款支付时间",         "source_table": "次表2", "source_field": "__COMPOUND_DELIVERY_PAY__",     "action": "直接取数", "note": "筛选付款类型=设计冻结款，取实际付款日期"},
        {"field": "结清款到期时间",         "source_table": "次表2", "source_field": "结清款到期日期",     "action": "直接取数", "note": "直接取数"},
        {"field": "质保到期时间",          "source_table": "次表2", "source_field": "__COMPOUND_WARRANTY__",        "action": "直接取数", "note": "质保开始日 + 备注中的质保期月数"},
        {"field": "数量",               "source_table": "次表4", "source_field": "__COMPOUND_RETRIEVAL__",       "action": "直接取数", "note": "筛选退库/出库，汇总数量"},
        {"field": "单位",               "source_table": "次表4", "source_field": "__COMPOUND_RETRIEVAL_UNIT__",  "action": "直接取数", "note": "筛选退库/出库，取第一个计量单位"},
    ],
    "改扩建工程": [
        {"field": "合同号",              "source_table": "主表",  "source_field": "合同编号",           "action": "直接取数", "note": "直接取数"},
        {"field": "合同名称",            "source_table": "主表",  "source_field": "合同名称",           "action": "直接取数", "note": "直接取数"},
        ...
        {"field": "现场最终要求交货日期(系统)", "source_table": "次表1", "source_field": "合同最终交货日期", "action": "直接取数", "note": "单个日期直接取，多个相同取重复值，多个不同取最早~最晚"},
        {"field": "实际到货日期",         "source_table": "次表3", "source_field": "实际到货日期",     "action": "直接取数", "note": "单个日期直接取，多个相同取重复值，多个不同取最早~最晚"},
        {"field": "全部到货验收时间",       "source_table": "主表",  "source_field": "__COMPOUND_ACCEPTANCE__",    "action": "直接取数", "note": "到货比例100%后取最晚验收日期"},
        {"field": "出入库时间",           "source_table": "次表4", "source_field": "__COMPOUND_WAREHOUSE__",      "action": "直接取数", "note": "筛选领料类型=领料，取日期"},
        {"field": "已付金额（元）",         "source_table": "主表",  "source_field": "已支付金额（元）",   "action": "直接取数", "note": "直接取数"},
        {"field": "累计付款比例(百分比)",    "source_table": "主表",  "source_field": "付款比例",           "action": "直接取数", "note": "直接取数"},
        {"field": "结清款到期时间",         "source_table": "次表2", "source_field": "结清款到期日期",     "action": "直接取数", "note": "直接取数"},
        {"field": "质保到期时间",          "source_table": "次表2", "source_field": "__COMPOUND_WARRANTY__",        "action": "直接取数", "note": "质保开始日 + 备注中的质保期月数"},
    ...
    if action == '直接取数':
        ...
        if '最早' in note and '最晚' in note:
            try:
                ...
                if len(dates) == 0:
                    return ""
                elif len(dates) == 1 or dates.nunique() == 1:
...
def build_result(main_df, sub_dfs, rules, category):
    ...
    field_map = {
        '合同号': '合同号', '合同名称': '合同名称', '厂家': '厂家',
        '数量': '数量', '数量_次表1': '数量', '数量_次表4': '逆向物资数量',
        '单位': '单位', '单位_次表1': '单位', '单位_次表4': '逆向物资单位',
        '合同签订金额': '合同签订金额', '合同签订金额（元）': '合同签订金额',
        '合同最终结算金额': '合同最终结算金额', '合同最终结算金额（元）': '合同最终结算金额',
        '付款比例': '付款比例', '合同签订日期': '合同签订日期',
        '投标偏差（是/否）': '投标偏差（是/否）',
        '合同要求到货时间': '合同要求到货时间',
        '已付金额（元）': '已付金额（元）',
        ...
        '累计付款比例': '累计付款比例(百分比)',
        '物资名称': '物资名称',
        '结清款到期时间': '结清款到期时间', '质保到期时间': '质保到期时间',
        '预付款支付时间': '预付款支付时间', '冻结款支付时间': '冻结款支付时间',
        '现场最终要求交货日期(系统)': '现场最终要求交货日期(系统)',
        '到货比例（%）': '到货比例（%）', '到货比例': '到货比例（%）',
        '已到货金额（元）': '已到货金额（元）',
        '全部到货验收时间': '全部到货验收时间', '实际到货日期': '实际到货日期',
    }
    ...
    accum_rate = raw.get('累计付款比例(百分比)', raw.get('累计付款比例', pd.Series('')))
    ...
    result['到货验收时间'] = raw.get('到货验收时间', pd.Series(''))
...
def _process_single_sheet(wb, result, summary_path, project_name):
    ...
    for _, r_row in result.iterrows():
        ...
        if contract in contract_row_map:
            ...
            row_changed = False
            for field in CONTRACT_INFO_FIELDS:
                if field in ('已付金额（元）', '供应商联系人及电话', '合同签订金额'):
                    continue
...
def process_project(main_df, sub_dfs, project_name, category, rules,
    ...
    try:
        main_name_col = find_field_in_df(main_df, ['项目名称', '工程名称', '工程项目名称'])
        ...
        for table_name, sub_df in sub_dfs.items():
            sub_name_col = find_field_in_df(sub_df, ['项目名称', '工程名称', '工程项目名称'])
            if sub_name_col:
            ...
            if not main_line.empty:
                ...
                for tn, sdf in sub_proj_dfs.items():
                    if tn == '次表1':
                        ...
                        if contract_col:
                            line_contracts = set(main_line['合同编号'].astype(str).str.strip())
                            sdf_copy = sdf.copy()
                            sdf_copy[contract_col] = sdf_copy[contract_col].astype(str).str.strip()
                            line_sub_dfs[tn] = sdf_copy[sdf_copy[contract_col].isin(line_contracts)]
                        else:
                            line_sub_dfs[tn] = sdf
            ...
            if not main_device.empty:
                ...
                for tn, sdf in sub_proj_dfs.items():
                    if tn == '次表1':
                        ...
                        contract_col = find_field_in_df(sdf, ['合同编号', '合同号'])
                        if contract_col:
                            device_contracts = set(main_device['合同编号'].astype(str).str.strip())
                            sdf_copy = sdf.copy()
                            sdf_copy[contract_col] = sdf_copy[contract_col].astype(str).str.strip()
                            device_sub_dfs[tn] = sdf_copy[sdf_copy[contract_col].isin(device_contracts)]
                        else:
