...
# ==================== 配置 ====================
# 规则已写死，无需再读取大模板文件
# 规则格式: {field, source_table, source_col_letter, action, note}
# action 类型: "直接取数" | "求和" | "多个日期取最早~最晚" | "多个日期取最晚" | 复合字段(通过 FIELD_NAME_OVERRIDES 映射)

HARDCODED_RULES = {
    "重特大工程": [
        {"field": "合同号",              "source_table": "主表",  "source_col_letter": "B",  "action": "直接取数",     "note": "直接取数"},
        {"field": "合同名称",            "source_table": "主表",  "source_col_letter": "C",  "action": "直接取数",     "note": "直接取数"},
        {"field": "厂家",               "source_table": "主表",  "source_col_letter": "K",  "action": "直接取数",     "note": "直接取数"},
        {"field": "数量",               "source_table": "次表1", "source_col_letter": "D",  "action": "求和",         "note": "一份合同可能对应多条物资名字，因此要对该合同下的所有数量进行求和"},
        {"field": "单位",               "source_table": "次表1", "source_col_letter": "K",  "action": "直接取数",     "note": "一般多条物资明细的单位均一致，直接取数"},
        {"field": "合同最终结算金额（元）", "source_table": "主表",  "source_col_letter": "E",  "action": "直接取数",     "note": "直接取数"},
        {"field": "付款比例",            "source_table": "主表",  "source_col_letter": "I",  "action": "直接取数",     "note": "直接取数"},
        {"field": "合同签订日期",         "source_table": "主表",  "source_col_letter": "AN", "action": "直接取数",     "note": "直接取数"},
        {"field": "投标偏差（是/否）",     "source_table": "主表",  "source_col_letter": "BM", "action": "直接取数",     "note": "若提出了正偏差，则填正偏差；若提出了负偏差，则填负偏差；如果未提出偏差则填否"},
        {"field": "合同要求到货时间",      "source_table": "主表",  "source_col_letter": "AU", "action": "直接取数",     "note": "直接取数"},
        {"field": "现场最终要求交货日期(系统)", "source_table": "次表1", "source_col_letter": "P",  "action": "直接取数",     "note": "如果明细中的日期为1个，则直接取数，如果有多个，则更新为最早时间~最晚时间"},
        {"field": "供货完成比例（%）",      "source_table": "主表",  "source_col_letter": "AA", "action": "直接取数",     "note": "直接取数"},
        {"field": "到货金额（万元）",       "source_table": "主表",  "source_col_letter": "AD", "action": "直接取数",     "note": "直接取数"},
        {"field": "全部到货验收时间",       "source_table": "次表3", "source_col_letter": "J",  "action": "直接取数",     "note": "如果一条合同下只有一个日期，则直接取数，如果有多个验收，则按最晚的时间更新"},
        {"field": "已付金额（元）",         "source_table": "主表",  "source_col_letter": "AE", "action": "直接取数",     "note": "直接取数"},
        {"field": "累计付款比例(百分比)",    "source_table": "主表",  "source_col_letter": "Y",  "action": "直接取数",     "note": "直接取数"},
        {"field": "预付款支付时间",         "source_table": "次表2", "source_col_letter": "E",  "action": "直接取数",     "note": "先从次表2-E列判断为预付款,再从次表2-AL列取数；如果没有数据则更新为/"},
        {"field": "冻结款支付时间",         "source_table": "次表2", "source_col_letter": "E",  "action": "直接取数",     "note": "先从次表2-E列判断为设计冻结款,再从次表2-AL列取数；如果没有数据则更新为/"},
        {"field": "结清款到期时间",         "source_table": "次表2", "source_col_letter": "BD", "action": "直接取数",     "note": "直接取数"},
        {"field": "质保到期时间",          "source_table": "次表2", "source_col_letter": "BC", "action": "直接取数",     "note": "当次表2-BC列为空时更新为/；不为空则按照次表2-BC列+主表-BM列中的质保期进行更新"},
        {"field": "数量",               "source_table": "次表4", "source_col_letter": "B",  "action": "直接取数",     "note": "次表4-B列筛选退库，再取F列数据；如果为空则更新为/"},
        {"field": "单位",               "source_table": "次表4", "source_col_letter": "B",  "action": "直接取数",     "note": "次表4-B列筛选退库，再取G列数据；如果为空则更新为/"},
    ],
    "改扩建工程": [
        {"field": "合同号",              "source_table": "主表",  "source_col_letter": "B",  "action": "直接取数",     "note": "直接取数"},
        {"field": "合同名称",            "source_table": "主表",  "source_col_letter": "C",  "action": "直接取数",     "note": "直接取数"},
        ...
        {"field": "合同要求到货时间",      "source_table": "主表",  "source_col_letter": "AU", "action": "直接取数",     "note": "直接取数"},
        {"field": "现场最终要求交货日期(系统)", "source_table": "次表1", "source_col_letter": "P",  "action": "直接取数",     "note": "如果明细中的日期为1个，则直接取数，如果有多个，则更新为最早时间~最晚时间"},
        {"field": "到货验收时间",          "source_table": "次表3", "source_col_letter": "J",  "action": "直接取数",     "note": "如果一条合同下只有一个日期，则直接取数，如果有多个验收，则按最晚的时间更新"},
        {"field": "出入库时间",           "source_table": "次表4", "source_col_letter": "B",  "action": "直接取数",     "note": "次表4-B列筛选领料，再取J列数据；如果为空则更新为/"},
        {"field": "已付金额（元）",         "source_table": "主表",  "source_col_letter": "AE", "action": "直接取数",     "note": "直接取数"},
        {"field": "累计付款比例(百分比)",    "source_table": "主表",  "source_col_letter": "Y",  "action": "直接取数",     "note": "直接取数"},
        {"field": "结清款到期时间",         "source_table": "次表2", "source_col_letter": "BD", "action": "直接取数",     "note": "直接取数"},
        {"field": "质保到期时间",          "source_table": "次表2", "source_col_letter": "BC", "action": "直接取数",     "note": "当次表2-BC列为空时更新为/；不为空则按照次表2-BC列+主表-BM列中的质保期进行更新"},
...
# skip_rows: pd.read_excel的skiprows参数
SOURCE_TABLE_CONFIG = {
...
# 字段名修正: 大模板列字母映射到源表字段名可能不正确，
# 手动指定输出字段 → {源表: 正确字段名}
# 仅用于修正列字母映射错误的情况
# 格式: {(字段名, 源表): 正确源字段名, ...}
# 特殊值: None = 跳过此源表, "__COMPOUND__" = 复合字段(需编程处理)
FIELD_NAME_OVERRIDES = {
    # 数量: 大模板说次表1-D列(合同编号), 实际是次表1-J列(采购数量)
    ("数量", "次表1"): "采购数量",
    # 合同要求到货时间: 大模板说主表-AU列, 主表AU=合同交货日期, 正确可用
    # 但为了明确, 也可从次表1取合同最终交货日期
    # ("合同要求到货时间", "主表"): None,
    # 次表4 数量(领用): 大模板说次表4-B列(领料类型), 实际是次表4-F列(数量)
    ("数量", "次表4"): "数量",
    # 次表4 单位(领用): 大模板说次表4-B列(领料类型), 实际是次表4-G列(计量单位)
    ("单位", "次表4"): "计量单位",
    # 预付款支付时间: 需从次表2-E筛"预付款"后取次表2-AL值
    ("预付款支付时间", "次表2"): "__COMPOUND_PREPAY__",
    # 冻结款/到货款支付时间: 需从次表2-E筛"到货款"后取次表2-AL值
    ("冻结款支付时间", "次表2"): "__COMPOUND_DELIVERY_PAY__",
    # 质保到期时间: 次表2-BC + 主表-BM复合
    ("质保到期时间", "次表2"): "__COMPOUND_WARRANTY__",
    # 出入库时间: filter 次表4-B="领料", get 次表4-J(领料时间)
    ("出入库时间", "次表4"): "__COMPOUND_WAREHOUSE__",
    # 物资名称: 大模板没有此字段，从次表1聚合
    ("物资名称", "次表1"): "__COMPOUND_MATERIALS__",
    # 次表4 数量: filter 次表4-B="退库/出库", 取F列(数量)
    ("数量", "次表4"): "__COMPOUND_RETRIEVAL__",
    # 次表4 单位: filter 次表4-B="退库/出库", 取G列(计量单位)
    ("单位", "次表4"): "__COMPOUND_RETRIEVAL_UNIT__",
}
...
# ==================== 源表读取与列名映射 ====================
def read_source_table(filepath, header_row, skip_rows=None):
    """读取源表Excel，返回 (DataFrame, {列字母: 字段名})。
    header_row: 列名所在行(1-based)
    skip_rows: 传给pd.read_excel
    """
    kwargs = {}
    ...
    for cell in ws[header_row]:
        if cell.value is not None and str(cell.value).strip():
            ...
            col_map[col_letter] = str(cell.value).strip()
    wb.close()

    return df, col_map


def read_sub_table2_all_sheets(filepath, header_row, skip_rows=None):
    """读取次表2所有sheet并拼接(延续表)"""
    ...
    for sn in xl.sheet_names:
        df = pd.read_excel(filepath, sheet_name=sn, **kwargs)
        df.columns = df.columns.astype(str).str.strip()
        df.columns = df.columns.str.replace('​', '')
        df.columns = df.columns.str.replace('（', '(').str.replace('）', ')')
        all_dfs.append(df)
...
def convert_col_letter_to_field(col_letter, col_map):
    """将列字母转换为字段名"""
    return col_map.get(col_letter, None)


# ==================== 构建源表字段名映射 ====================
def build_source_field_mapping(rules, source_col_maps):
    """将规则中的列字母转换为实际字段名。
    source_col_maps: {源表名: {列字母: 字段名}}
    返回: rules增强版，增加 source_field 字段
    """
    # 先保存原始字段名
    for rule in rules:
        rule['_original_field'] = rule['field']

    # 处理重复字段名: 统计出现次数，重复的加后缀
    field_counts = {}
    for rule in rules:
    ...
    for rule in rules:
        ...
        col_letter = rule['source_col_letter']
        orig_field = rule['_original_field']
        col_map = source_col_maps.get(src_table, {})
        field_name = convert_col_letter_to_field(col_letter, col_map)

        # 查找手动修正
        override_key = (orig_field, src_table)
        if override_key in FIELD_NAME_OVERRIDES:
            field_name = FIELD_NAME_OVERRIDES[override_key]

        rule['source_field'] = field_name
...
def extract_contract_data(main_df, sub_dfs, rules):
    ...
    for contract in contracts:
        ...
        # 从主表获取该合同数据(取第一条)
        main_match = main_df[main_df['合同编号'] == contract]
        ...
        for rule in rules:
            ...
            # 检查是否为复合字段
            if src_field and str(src_field).startswith('__COMPOUND_'):
...
def _apply_compound_rule(contract, main_df, sub_dfs, compound_type):
    ...
    if compound_type == '__COMPOUND_PREPAY__':
        ...
        # 次表2-E筛选"到货款"或"投运款"，取次表2-AL(实际付款日期)
        sub2 = sub_dfs.get('次表2')
        if sub2 is None:
            return ''
        contract_col = find_field_in_df(sub2, ['合同编号', '合同号'])
        ...
        if not all([contract_col, type_col, date_col]):
            return ''
        match = sub2[(sub2[contract_col].astype(str).str.strip() == str(contract).strip()) &
                     (sub2[type_col].astype(str).str.contains('到货款|投运款'))]
        if match.empty:
            return '/'
        dates = pd.to_datetime(match[date_col], errors='coerce').dropna()
        return dates.iloc[0].strftime('%Y-%m-%d') if not dates.empty else '/'
        ...
        # 次表2-BC(质保期开始计算之日) + 主表-BM(备注)中质保期
        sub2 = sub_dfs.get('次表2')
        ...
        if match.empty:
            ...
            if main_contract_col:
                ...
                if not mm.empty:
                    ...
                    if bm_col:
                        return str(mm[bm_col].iloc[0]) if pd.notna(mm[bm_col].iloc[0]) else '/'
            return '/'
...
def build_result(main_df, sub_dfs, rules, category):
    ...
    # 字段映射: 规则输出字段 → CONTRACT_INFO_FIELDS
    # 去重后的规则字段: 数量_次表1, 数量_次表4 等
    field_map = {
        '合同号': '合同号',
        '合同名称': '合同名称',
        '厂家': '厂家',
        '数量': '数量',  # 合同数量，规则中第一次出现的数量(来自次表1)
        '数量_次表1': '数量',
        '数量_次表4': '逆向物资数量',
        '单位': '单位',
        '单位_次表1': '单位',
    ...
    # 合同最终结算金额
    result['合同最终结算金额'] = raw.get('合同最终结算金额（元）',
                                    raw.get('合同最终结算金额', pd.Series('')))
    ...
    result['合同签订日期'] = signing_date.apply(safe_date_parse)

    # 供应商联系人及电话: 从主表取(模板标记为人工更新)
    ...
    # 结清款到期时间：从次表2 BD列(结清款到期日期)直接取数
    result['结清款到期时间'] = raw.get('结清款到期时间', pd.Series(''))

    # 质保到期时间：从次表2 BC列(质保期开始计算之日) + 主表备注复合取数
                ...
                for tn, sdf in sub_proj_dfs.items():
                    if tn == '次表1':
                        ...
                        if contract_col:
                            ...
                            sdf_copy = sdf.copy()
                            sdf_copy[contract_col] = sdf_copy[contract_col].astype(
                                str).str.strip()
...
class App:
    def __init__(self, root):
        ...
        self.project_data = []  # [(name, category), ...]
    ...
    def run(self):
        ...
        try:
            ...
            main_df, main_col_map = read_source_table(main_path,
                                                      **SOURCE_TABLE_CONFIG['主表'])
            sub1_df, sub1_col_map = read_source_table(sub1_path,
                                                      **SOURCE_TABLE_CONFIG['次表1'])
            sub2_df, sub2_col_map = read_sub_table2_all_sheets(sub2_path,
                                                               **SOURCE_TABLE_CONFIG['次表2'])
            sub3_df, sub3_col_map = read_source_table(sub3_path,
                                                      **SOURCE_TABLE_CONFIG['次表3'])
            sub4_df, sub4_col_map = read_source_table(sub4_path,
                                                      **SOURCE_TABLE_CONFIG['次表4'])
            ...
            for category, raw_rules in HARDCODED_RULES.items():
                rules_cache[category] = build_source_field_mapping(
                    raw_rules, source_col_maps)
