...
def process_project(main_df, sub_df, project_name, summary_path):
    ...
    try:
        ...
        result['合同签订日期'] = merged['合同签约完成时间']          # 直接使用
    ...
    def run(self):
        ...
        try:
            ...
            for proj in projects:
                original_path = find_summary_file(folder, proj)
                if not original_path:
                    fail.append(f"{proj}: 未找到汇总表文件")
                    continue
                # 复制到新文件夹
                new_path = os.path.join(output_dir, os.path.basename(original_path))
