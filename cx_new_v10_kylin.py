...
# ==================== 麒麟/Linux 适配 ====================
# 字体：麒麟系统没有微软雅黑，自动检测可用的中文字体
def _detect_font():
    """检测系统可用的中文字体，返回第一个可用的字体名"""
...
# 文件对话框类型过滤
# Linux/tkinter 不同桌面环境对 filter 格式兼容性不同，拆成多条保证兼容
if sys.platform == 'win32':
...
def process_project(main_df, sub_dfs, project_name, category, rules,
    ...
    try:
        ...
        # v10: 排除主表中"合同状态=合同终止"的合同——新增时跳过，已存在于模板中的一并删除
        status_col = find_field_in_df(main_proj, ['合同状态'])
        if status_col is not None:
            term_mask = main_proj[status_col].fillna('').astype(str).str.strip() == '合同终止'
            if term_mask.any():
                term_nos = main_proj.loc[term_mask, '合同编号'].fillna('').astype(str).str.strip()
                # 有编号的用于删除模板里已存在的行；空编号的（零星采购类）靠下面的行级过滤剔除
                excluded_contracts |= {n for n in term_nos.tolist() if n}
                main_proj = main_proj[~term_mask].copy()
...
class App:
    def __init__(self, root):
        ...
        tk.Label(root, text="工程物资一本账更新工具", font=(FONT_NAME, 14, "bold"),
        ...
        tk.Label(time_frame, text="（用于输出文件名）",
                 bg="#f0f0f0", fg="#555555", font=(FONT_NAME, 9)).pack(side="left", padx=5)
        ...
        self.listbox = tk.Listbox(list_frame, selectmode=tk.MULTIPLE, yscrollcommand=scroll.set,
                                  font=(FONT_NAME, 10), width=60, height=8)
        ...
        self.run_btn = tk.Button(root, text="开始更新汇总表", command=self.run,
                                 bg="#4CAF50", fg="white", font=(FONT_NAME, 12, "bold"),
                                 width=20, height=2)
        ...
        tk.Label(filef, text="(可多选已生成的汇总表xlsx)",
                 bg="#f0f0f0", fg="#555555", font=(FONT_NAME, 9)).pack(side="left")
        ...
        self.report_listbox = tk.Listbox(rlist_frame, height=3,
                                         yscrollcommand=rscroll.set, font=(FONT_NAME, 10))
        ...
        self.report_btn = tk.Button(btn_rows, text="AI生成报告", command=self.run_report,
                                    bg="#2196F3", fg="white", font=(FONT_NAME, 11, "bold"),
                                    width=20)
        self.report_btn.pack(side="left", padx=5)
        tk.Button(btn_rows, text="打开输出目录", command=self.open_report_dir,
                  font=(FONT_NAME, 10), width=14).pack(side="left", padx=5)
        ...
        tk.Label(root, textvariable=self.report_status_var, bg="#f0f0f0",
                 fg="#2196F3", font=(FONT_NAME, 9)).pack(pady=1)
    ...
    def browse_file(self, var):
        if sys.platform == 'win32':
            path = filedialog.askopenfilename(filetypes=FILE_FILTER)
        else:
            path = self._native_file_dialog('选择Excel文件', '*.xlsx *.xls')
        if path:
            var.set(path)

    def _native_file_dialog(self, title, pattern):
        """麒麟/Linux使用zenity原生文件对话框(单选)"""
        try:
            result = subprocess.run(
                ['zenity', '--file-selection', '--title', title,
                 '--file-filter', f'{pattern} | {pattern}'],
            ...
            pass
        # zenity失败则回退到tkinter
        return filedialog.askopenfilename(filetypes=FILE_FILTER)

    def _native_files_dialog(self, title, pattern):
        """麒麟/Linux使用zenity原生文件对话框(多选)"""
        try:
            result = subprocess.run(
            ...
            pass
        # zenity失败则回退到tkinter
        return list(filedialog.askopenfilenames(filetypes=FILE_FILTER))

    def _native_folder_dialog(self, title):
        """麒麟/Linux使用zenity原生文件夹对话框"""
        try:
            result = subprocess.run(
    ...
    def browse_folder(self, var):
        ...
        if sys.platform == 'win32':
            ...
            path = self._native_folder_dialog('选择汇总表文件夹')
        if path:
            var.set(path)
    ...
    def pick_report_files(self):
        if sys.platform == 'win32':
            ...
            paths = self._native_files_dialog(
                '选择已生成的汇总表文件(可多选)', '*.xlsx *.xls')
        if paths:
