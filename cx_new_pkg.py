...
# ==================== 配置 ====================
# 大模板路径（相对于脚本目录或绝对路径）
# PyInstaller 打包后 sys._MEIPASS 指向内部资源目录
...
BIG_TEMPLATE_CONFIG = {
    "重特大工程": {
        "path": os.path.join(BASE_DIR, "汇总表模板", "1.重特大工程模板-设备物资管理一本账20260518.xlsx"),
        "header_rows": [2, 3],
        "source_row": 4,
        "note_row": 5,
        "data_start": 6,
    ...
    "改扩建工程": {
        "path": os.path.join(BASE_DIR, "汇总表模板", "2.改扩建工程模板-物资管理一本账20260526.xlsx"),
        "header_rows": [3, 4],
        "source_row": 5,
        "note_row": 6,
        "data_start": 7,
...
# ==================== 大模板解析 ====================
def parse_big_template(filepath, header_rows, source_row, note_row):
    """解析大模板，提取每个字段的数据来源规则。
    返回: [{field, col_idx, source_table, source_col_letter, action, note}, ...]
    不含"人工更新"和"删除该字段"的列。
    """
    wb = openpyxl.load_workbook(filepath)
    ws = wb.active

    # 读取每列字段名(取header_rows中最后有值的)
    field_names = []
    ...
    sources = []
    for col in range(1, ws.max_column + 1):
        v = ws.cell(row=source_row, column=col).value
        sources.append(str(v).strip() if v is not None else '')

    notes = []
    for col in range(1, ws.max_column + 1):
        v = ws.cell(row=note_row, column=col).value
        notes.append(str(v).strip() if v is not None else '')
    ...
    for i in range(len(field_names)):
        ...
        m = re.match(r'(主表|次表\d+)-([A-Z]+)列(.*)', src)
        if m:
            source_table = m.group(1)
            source_col = m.group(2)
            action = m.group(3).strip() if m.group(3).strip() else '直接取数'
...
def build_result(main_df, sub_dfs, rules, category):
    ...
    # 投标偏差
    result['投标偏差（是/否）'] = raw.get('投标偏差（是/否）', pd.Series(''))
...
class App:
    ...
    def run(self):
        ...
        try:
            ...
            # 解析两个大模板规则
            rules_cache = {}
            for category, config in BIG_TEMPLATE_CONFIG.items():
                if os.path.exists(config['path']):
                    raw_rules = parse_big_template(
                        config['path'],
                        config['header_rows'],
                        config['source_row'],
                        config['note_row'],
            ...
            for proj_name, proj_category in projects:
                ...
                if not rules:
                    cfg = BIG_TEMPLATE_CONFIG.get(proj_category, {})
                    tpl_path = cfg.get('path', '未知路径')
                    fail.append(f"{proj_name}: 未找到{proj_category}的大模板规则\n  大模板路径: {tpl_path}\n  请检查大模板文件是否存在")
                    continue
